...
dependencies {
    ...
    implementation("org.springframework.boot:spring-boot-starter-data-jpa:${Deps.springBoot.version}")
    ...
    main {
        java {
            srcDirs(
                ...
                "$projectDir/build/generated-daml-bindings"
            )
            // Exclude CantonTreasuryService when Daml codegen is not available
            // It references treasury.* types generated by `make build-daml`
            if (!file("$projectDir/build/generated-daml-bindings").exists()) {
                exclude("com/digitalasset/quickstart/service/CantonTreasuryService.java")
            }
        }
    }
    test {
